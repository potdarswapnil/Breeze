...
lapply(c("drc", "caTools", "ggplot2", "gsubfn", "gtools", "data.table", "doSNOW","stringr"), library, character.only = !0)
...
CALC_IC50_EC50_DSS <- compiler::cmpfun(function(i, drug_wells_, xpr_tbl, DSS_typ, readoutCTX = F)
  {
    ...
    tryCatch({
      ...
      if(DSS_typ == "AUC"){
        ...
        dose_response_fit=data.frame(screen=screen,product_id=product_id,drug_name=drug_name,x=x,yic=yic,ytec=ytec,Readout=Readout)
        #return list with 3 nodes - 1 row for IC50 table and 1 row for EC50 table and EC50 image in base64
        list(IC50_dataframe, TEC50_dataframe, TEC50base64, believe_, coef_ic50ABS)
      }
    }, error = function(e) {
